...
import { FaMicrophone, FaTimes, FaPaperPlane, FaVolumeUp } from 'react-icons/fa';
import { useVapiClient, startVapiCall, stopVapiCall, sendVapiMessage } from '../lib/vapi.sdk';

export default function VoiceAssistant() {
    const [isOpen, setIsOpen] = useState(false);
    const [messages, setMessages] = useState([
        { text: "Hello! I'm your mental health assistant. How can I help you today?", sender: 'assistant' }
    ]);
    ...
    useEffect(() => {
        ...
        const handleMessage = (message) => {
            ...
            else if (message.type === "transcript" && message.transcript?.text && message.role === "user") {
            ...
            else if (message.type === "transcript" && message.transcript?.text && message.role === "assistant") {
                console.log("Processing assistant transcript:", message.transcript.text, message.transcriptType);
                ...
                if (message.transcriptType === "final") {
                    ...
                    setMessages(prev => {
                        ...
                        const isDuplicate = prev.some(m => 
                            m.sender === 'assistant' && m.text === newText
                        );
            ...
            // Handle conversation updates (contains messages)
                ...
                const newMessages = message.messages.filter(msg => 
            ...
            else if (message.type === "assistant-message" && message.message?.content) {
                setMessages(prev => [...prev, { 
    ...
    const startCall = async () => {
        ...
        try {
            ...
            const call = await startVapiCall(client, process.env.NEXT_PUBLIC_VAPI_ASSISTANT_ID);
            setActiveCall(call);
            setIsProcessing(false);
        } catch (error) {
            ...
            {isOpen ? (
                <div className="bg-white rounded-lg shadow-xl w-96 h-[500px] flex flex-col">
                    ...
                    <div className="bg-purple-600 text-white p-4 rounded-t-lg flex justify-between items-center">
                        ...
                        <button onClick={handleClose} className="hover:bg-purple-700 p-1 rounded">
                            <FaTimes />
                        </button>
                    </div>
                            ...
                            >
                                ...
                                {message.sender === 'assistant' && (
                                    <div className="mt-1 text-xs text-gray-500 flex items-center">
                                        <FaVolumeUp className={`mr-1 ${isSpeaking ? 'text-purple-600' : ''}`} />
